...
test('render book', () => {
  ...
  expect(getByText(`Title: ${book.title}`)).toBeInTheDocument();
  expect(getByText(`Short description: ${book.shortDescription}`)).toBeInTheDocument();
  expect(getByText(`Pages: ${book.pages}`)).toBeInTheDocument();
  expect(getByText(`Language: ${book.language}`)).toBeInTheDocument();
  expect(getByText(`Progress: ${book.progress}%`)).toBeInTheDocument();
  expect(getByText(`Min price: ${book.minPrice}$`)).toBeInTheDocument();
  expect(getByText(`Desired price: ${book.desiredPrice}$`)).toBeInTheDocument();
  expect(getByText(`Current sum: ${book.currentSum}$`)).toBeInTheDocument();
  expect(getByText(`Expected sum: ${book.expectedSum}$`)).toBeInTheDocument();
});
